...
public class MedagliereMain {
	...
	// opzioni del men� iniziale del programma uggg
	...
	public static void main(String[] args) {
		...
		while (ciclo) {
			...
			case 3: // aggiungi risultato gara
				...
				// stampa gare alle quali � possibile inserire un risultato
				...
				// se l'elenco delle nazioni � vuoto
				if (medagliere.getNazioni().size() <= 0)
					System.out.println(ERR_ELENCO_VUOTO);

				// se la gara selta � a squadre e ci sono meno di 3 nazioni in
				// elenco nazioni
				...
				// se la gara inserita � contenuta nell'elenco delle gare ancora
				// senza vincitori
					...
					do {
						...
						// se non � contenuta in medagliere
					...
					do {
						...
						// se non � contenuta nel medagliere
						if (!medagliere.getNazioni().contains(argento)) {
							System.out.println(ERR_NAZIONE_NON_ESISTE);

						} else {
							// se la gara � a squadre ed � stata inserita 2
					...
					do {
						...
						// se non � presente in medagliere
						if (!medagliere.getNazioni().contains(bronzo)) {
							System.out.println(ERR_NAZIONE_NON_ESISTE);

						} else {
							// se la gara � a squadre e sono state inserite due
							// nazioni uguali
							if ((gare.getElenco().get(gare.getElenco().indexOf(g)).getSquadra())
