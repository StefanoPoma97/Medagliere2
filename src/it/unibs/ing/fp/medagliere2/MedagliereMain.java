...
public class MedagliereMain {
	...
	// opzioni del menï¿½ iniziale del programma uggg
	public static final String[] VOCI_MENU_INIZIALE = { "Inserire nuova nazione", "Inserire nuova gara",
			"Inserire risultato gara", "Visualizzare medagliere" };
	...
	public static final String ERR_NAZIONE_PRESENTE = "Nazione giÃ  presente!";
	...
	public static final String ERR_ELENCO_VUOTO = "L'elenco delle nazioni Ã¨ vuoto! Non si possono inserire vincitori";
		...
		while (ciclo) {
			...
			case 2: // aggiungi gara
				...
				if (!gare.getElenco().contains(gara)) {
					...
					if (Character.toLowerCase(squadra) == 's')
						gara.setSquadra(true);

				}

				else
					System.out.println("Gara giÃ  presente");
			...
			case 3: // aggiungi risultato gara
				...
				// stampa gare alle quali ï¿½ possibile inserire un risultato
				...
				// elenco nazioni
				else if ((gare.getElenco().get(gare.getElenco().indexOf(g)).getSquadra())
						&& (medagliere.getNumeroNazioni() < 3))
					System.out.println(ERR_MENO3_NAZIONI);

				// se la gara inserita ï¿½ contenuta nell'elenco delle gare ancora
				// senza vincitori
					...
					do {
						...
						// se non ï¿½ contenuta in medagliere
					...
					do {
						...
						// se non ï¿½ contenuta nel medagliere
						if (!medagliere.getNazioni().contains(argento)) {
							System.out.println(ERR_NAZIONE_NON_ESISTE);

						} else {
							// se la gara ï¿½ a squadre ed ï¿½ stata inserita 2
							// volte la stessa nazione
							if ((gare.getElenco().get(gare.getElenco().indexOf(g)).getSquadra())
									&& (argento.equals(oro)))
					...
					do {
						...
						// se non ï¿½ presente in medagliere
						if (!medagliere.getNazioni().contains(bronzo)) {
							System.out.println(ERR_NAZIONE_NON_ESISTE);

						} else {
							// se la gara ï¿½ a squadre e sono state inserite due
							// nazioni uguali
							if ((gare.getElenco().get(gare.getElenco().indexOf(g)).getSquadra())
									&& (bronzo.equals(oro) || bronzo.equals(argento)))
								System.out.println(ERR_NAZIONE_PRESENTE2_VOLTE);
							else {
								repeat = false;
